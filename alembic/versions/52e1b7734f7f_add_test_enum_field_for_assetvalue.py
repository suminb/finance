...
old_type = postgresql.ENUM("yahoo", "google", "kofia", name="asset_value_source")


def upgrade():
    op.alter_column(
        "asset_value", "source", type_=new_type, existing_type=old_type, nullable=False
    )


def downgrade():
    op.alter_column(
        "asset_value", "source", type_=old_type, existing_type=new_type, nullable=False
    )
