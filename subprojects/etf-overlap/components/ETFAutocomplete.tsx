...
export default function ETFAutocomplete({ selectedETFs, onChange, disabled }: ETFAutocompleteProps) {
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      try {
        const response = await fetch(`/api/search-etfs?q=${encodeURIComponent(inputValue)}`)
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
    ...
    } else if (e.key === 'Backspace' && inputValue === '' && selectedETFs.length > 0) {
    ...
    <div style={{ position: 'relative', width: '100%' }}>
      ...
      >
        ...
        <input
          ...
          placeholder={selectedETFs.length === 0 ? 'Type ETF ticker or name (e.g., SPY, QQQ)...' : ''}
          disabled={disabled}
          style={{
            flex: '1',
            minWidth: '200px',
