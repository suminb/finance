  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
      ...
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontSize: '0.875rem', color: '#6b7280' }}>Core Overlap</div>
...
export default function OverlapPage() {
  ...
  const handleCellClick = (etf1: string, etf2: string) => {
    ...
    if (detail) {
      ...
      setTimeout(() => {
        document.getElementById('overlap-details')?.scrollIntoView({ behavior: 'smooth' })
      }, 100)
    }
  }
      ...
      {data && data.matrix && data.matrix.length > 0 && (
        <div>
          ...
          {data.coreOverlap && data.etfs.length >= 2 && (
            <div style={{ marginBottom: '3rem', padding: '2rem', backgroundColor: '#f0f9ff', borderRadius: '12px', border: '2px solid #0070f3' }}>
              ...
              <div style={{ display: 'flex', gap: '3rem', alignItems: 'center', flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: '1', minWidth: '300px' }}>
                  <div style={{ display: 'grid', gap: '1rem' }}>
                    <div>
                      <div style={{ fontSize: '0.875rem', color: '#6b7280' }}>Total Core Overlap</div>
                    ...
                    <div>
                      <div style={{ fontSize: '0.875rem', color: '#6b7280' }}>Shared Holdings</div>
              ...
              {data.coreOverlap.sharedHoldings.length > 0 && (
                <div style={{ marginTop: '2rem' }}>
                  ...
                  <div style={{ overflowX: 'auto', backgroundColor: 'white', borderRadius: '8px', padding: '1rem' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead>
                        <tr style={{ backgroundColor: '#f5f5f5' }}>
                          ...
                          {data.etfs.map(etf => (
                            <th key={etf} style={{ padding: '0.75rem', textAlign: 'right', borderBottom: '2px solid #ddd' }}>
                              {etf} %
                            </th>
                          ))}
                          <th style={{ padding: '0.75rem', textAlign: 'right', borderBottom: '2px solid #ddd', backgroundColor: '#eff6ff' }}>
                            Min %
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {data.coreOverlap.sharedHoldings.map((holding, index) => (
                          <tr key={holding.symbol} style={{ borderBottom: '1px solid #eee' }}>
                            ...
                            {data.etfs.map(etf => (
                              <td key={etf} style={{ padding: '0.75rem', textAlign: 'right' }}>
                            ...
                            <td style={{ 
                              padding: '0.75rem', 
                              textAlign: 'right', 
                              backgroundColor: index < 10 ? '#eff6ff' : 'transparent',
                      ...
                      <tfoot>
                        <tr style={{ backgroundColor: '#f5f5f5', fontWeight: 'bold' }}>
                          <td colSpan={2 + data.etfs.length} style={{ padding: '0.75rem', textAlign: 'right' }}>
                            Total Core Overlap:
                          </td>
                          <td style={{ padding: '0.75rem', textAlign: 'right', color: '#0070f3', fontSize: '1.1rem' }}>
                            {data.coreOverlap.totalOverlap.toFixed(2)}%
                          </td>
                        </tr>
                      </tfoot>
              ...
              {data.coreOverlap.sharedHoldings.length === 0 && (
                <div style={{ marginTop: '2rem', padding: '1.5rem', backgroundColor: '#fff3cd', borderRadius: '8px', border: '1px solid #ffc107' }}>
                  <p style={{ margin: 0, color: '#856404' }}>
                    ⚠️ No holdings are shared across all {data.etfs.length} ETFs. They have completely different portfolios.
                  </p>
                </div>
              )}
            </div>
          ...
          <p style={{ marginBottom: '1.5rem', color: '#666' }}>
            Values show weighted overlap percentage (0-100%). Darker colors indicate higher overlap. Click any cell for details.
          </p>
                  ...
                  <tr key={etf1}>
                    ...
                    {data.matrix[i].map((value, j) => (
                      <td
                        ...
                        title={i === j ? `${etf1}: 100%` : `Click to see ${etf1} vs ${data.etfs[j]} overlap details`}
                        onMouseEnter={(e) => {
                          if (i !== j) {
          ...
          <div style={{ marginTop: '2rem', padding: '1rem', backgroundColor: '#eff6ff', borderRadius: '8px', border: '1px solid #bfdbfe' }}>
            ...
            <ul style={{ marginLeft: '1.5rem', color: '#1e40af', lineHeight: '1.6' }}>
              ...
              <li><strong>💡 Click any cell</strong> to see detailed overlapping holdings</li>
            </ul>
          </div>

          {/* Detailed overlap view */}
          {selectedDetail && (
            <div id="overlap-details" style={{ marginTop: '3rem', padding: '2rem', backgroundColor: '#ffffff', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
                <div style={{ padding: '1rem', backgroundColor: '#f9fafb', borderRadius: '8px' }}>
                  <div style={{ fontSize: '0.875rem', color: '#6b7280' }}>Total Overlap</div>
                ...
                <div style={{ padding: '1rem', backgroundColor: '#f9fafb', borderRadius: '8px' }}>
                  <div style={{ fontSize: '0.875rem', color: '#6b7280' }}>Unique to {selectedDetail.etf1}</div>
                  <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>
                    {selectedDetail.uniqueHoldings1}
                  </div>
                </div>
                <div style={{ padding: '1rem', backgroundColor: '#f9fafb', borderRadius: '8px' }}>
                  <div style={{ fontSize: '0.875rem', color: '#6b7280' }}>Unique to {selectedDetail.etf2}</div>
                  <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {selectedDetail.sharedHoldings.map((holding, index) => (
                      <tr key={holding.symbol} style={{ borderBottom: '1px solid #eee' }}>
                        ...
                        <td style={{ padding: '0.75rem', textAlign: 'right', backgroundColor: index < 10 ? '#eff6ff' : 'transparent', fontWeight: index < 5 ? 'bold' : 'normal' }}>
                  ...
                  <tfoot>
                    <tr style={{ backgroundColor: '#f5f5f5', fontWeight: 'bold' }}>
                      ...
                      <td style={{ padding: '0.75rem', textAlign: 'right', color: '#0070f3' }}>
                        {selectedDetail.overlapPercentage.toFixed(2)}%
                      </td>
                    </tr>
                  </tfoot>
