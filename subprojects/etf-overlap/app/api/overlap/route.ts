...
function calculateCoreOverlap(
  holdingsMap: Map<string, Array<{ symbol: string; name: string; weight: number }>>,
  tickers: string[]
): CoreOverlap {
  if (tickers.length < 2) {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const holdingsMap = new Map<string, Array<{ symbol: string; name: string; weight: number }>>()
    ...
    return NextResponse.json(
      { 
        error: error instanceof Error ? error.message : 'Failed to calculate overlap',
        etfs: tickers,
        matrix: []
      },
      { status: 500 }
