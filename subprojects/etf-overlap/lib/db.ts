...
export function getPool(): Pool {
  if (!pool) {
    ...
    if (!databaseUrl) {
      throw new Error('DATABASE_URL or SBF_DB_URL environment variable is not set')
    }
...
export async function saveHoldings(
  ...
  try {
    ...
    // Delete old holdings
    await client.query(
      'DELETE FROM etf_holdings WHERE etf_id = $1',
      [etfId]
    )

    // Insert new holdings
    if (holdings.length > 0) {
      const values = holdings.map((holding, idx) => {
...
export async function saveSnapshot(
  ...
  try {
    ...
    if (etfResult.rows.length === 0) {
      throw new Error(`ETF ${ticker} not found in database. Save holdings first.`)
    }
    ...
    await client.query('COMMIT')
    console.log(`Saved snapshot of ${holdings.length} holdings for ${ticker} on ${dateStr}`)
  } catch (error) {
    await client.query('ROLLBACK')
