# NOTE: We are in the way of making all frontend stuff Jupyter presentable. We
# would like to remove this module entirely if possible.
...
class AdminModelView(ModelView):
    def is_accessible(self):
        return not current_user.is_anonymous and current_user.email in ADMINS


def create_app(
    name=__name__, config=None, static_folder="assets", template_folder="templates"
):
    ...
    classes = [Account, Asset, AssetValue, Portfolio, Record, Transaction, User]
    for cls in classes:
        admin.add_view(ModelView(cls, db.session, endpoint=cls.__name__))
